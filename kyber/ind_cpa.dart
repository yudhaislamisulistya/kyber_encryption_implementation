...
class INDCPA {
  static RejectionSamplingResult rejectionSamplingOnUniformRandomBytes(
    ...
    while (ctr < len && pos + 3 <= bufl) {
      val0 = (KyberFunctions.uint16((buf[pos]) >> 0) | (KyberFunctions.uint16(buf[pos + 1]) << 8)) & 0xFFF;
      val1 = (KyberFunctions.uint16((buf[pos + 1]) >> 4) | (KyberFunctions.uint16(buf[pos + 2]) << 4)) & 0xFFF;
      pos = pos + 3;
      if (val0 < paramsQ) {
        r[ctr] = val0;
        ctr = ctr + 1;
  ...
  static List<List<int>> generateKeys(KyberLevel level) {
    ...
    for (int i = 0; i < paramsK(level); i++) {
      ...
      pk[i] = KyberFunctions.polynomialToMontgomeryDomain(NTT.pointwiseMultiply(level, a[i], s));
    }
    for (int i = 0; i < paramsK(level); i++) {
  ...
  static List<int> encrypt(
    ...
    List<int> seed = pk1.sublist(start, end);
  ...
  static List<int> decrypt(KyberLevel level, List<int> c, List<int> privateKey) {
    var u = KyberFunctions.decompress1(level, c.sublist(0, 1408)).map((e) => e.noNull()).toList();
    var v = KyberFunctions.decompress2(c.sublist(1408, 1568));
    var privateKeyPolyvec = KyberFunctions.bytesToVectorOfPolynomials(level, privateKey);
    for (int i = 0; i < paramsK(level); i++) {
